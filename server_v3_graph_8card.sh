#!/usr/bin/env bash
# V3 graph path (GLM5_KDA_VERIFY_V3=1, V2 off, graph on) — fused multi-slot
...
nohup numactl -C $((DEVICE*CORES_PER_CARD))-$((DEVICE*CORES_PER_CARD+CORES_PER_CARD-1)) $XLLM_PATH \
  ...
  --draft_model="$DRAFT_PATH" --num_speculative_tokens=1 \
  --speculative_algorithm=MTP --enable_graph=True --model_impl=python \
