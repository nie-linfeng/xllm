...
MTPWorkerImpl::MTPWorkerImpl(const ParallelArgs& parallel_args,
  ...
  (void)enable_adaptive_speculative_decode;  // adaptive controller not wired on this branch
...
std::optional<ForwardOutput> MTPWorkerImpl::run_validate(
  ...
  for (const ForwardOutput& draft_output : draft_outputs) {
    target_output.retained_inputs.insert(
        target_output.retained_inputs.end(),
        draft_output.retained_inputs.begin(),
        draft_output.retained_inputs.end());
  }
